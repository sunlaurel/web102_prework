...
const unfundedCount = GAMES_JSON.reduce((acc, game) => game.pledged < game.goal ? acc + 1 : acc, 0);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0).toLocaleString('en-US')} has been
raised for ${GAMES_JSON.length} games. Currently, ${unfundedCount} game${unfundedCount == 1 ? " remains": "s remain"}
